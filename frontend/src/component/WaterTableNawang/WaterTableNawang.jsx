...
import "./WaterTableNawang.css";

const WaterTableNawang = ({ dateRange }) => { 
  ...
  // ฟังก์ชันแปลงรูปแบบวันที่สำหรับ API
  const formatDateForAPI = (dateTimeLocal) => {
    if (!dateTimeLocal) return '';
    // แปลงจาก "2024-01-01T10:30" เป็น "2024-01-01 10:30:00"
    return dateTimeLocal.replace('T', ' ') + ':00';
  };

  const fetchData = (startDate = '', endDate = '') => {
    setLoading(true);
    let apiUrl = "https://myweb.ac.th/get_water_dataNawang.php";
    ...
    console.log('Fetching from:', apiUrl); // สำหรับ debug
  ...
  const handleExportCSV = () => {
    const header = ["วันที่/เวลา", "ระดับน้ำ (ม.รทก.)", "อัตราการไหล (ลบ.ม./วินาที)"];
  ...
  return (
    <section className="water-table-section">
      <h4>สถานี : สะพานมิตรภาพแม่นาวาง-ท่าตอน</h4>
      ...
      <div className="table-wrapper">
        <table className="water-table">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="3" style={{ textAlign: 'center' }}>กำลังโหลดข้อมูล...</td></tr>
            ) : groupedData.length > 0 ? (
              groupedData.map((item, idx) => {
                if (item.type === 'date') {
                  return (
                    <tr key={`date-${item.date}`} className="date-separator">
                      <td colSpan="3" className="date-cell">{item.date}</td>
                    </tr>
                  );
                }
                return (
              ...
              <tr><td colSpan="3" style={{ textAlign: 'center' }}>ไม่พบข้อมูลในช่วงวันที่ที่เลือก</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
export default WaterTableNawang;